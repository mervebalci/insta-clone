...
function useCreatePost() {
  ...
  async function handleCreatePost(selectedFile, caption) {
    ...
    try {
      // Create post doc and add the post image to the storage
      const postDocRef = await addDoc(collection(firestore, "posts"), newPost);
      const userDocRef = doc(firestore, "users", authUser.uid);
      const imageRef = ref(storage, `posts/${postDocRef.id}`);

      await updateDoc(userDocRef, {posts: arrayUnion(postDocRef.id)});
      await uploadString(imageRef, selectedFile, "data_url");   // To upload the img
      const downloadURL = await getDownloadURL(imageRef);   // To download the URL
      
      // This post doesn't have any img URL and we'd like to update that both in the database and frontend.
